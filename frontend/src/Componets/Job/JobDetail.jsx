...
// import Jobs from "../../../../backend/Data/Data/JobsDataAvl";
...
function JobDetail() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const response = await axios.get(`http://localhost:10000/api/job/${id}`);
  ...
  const submitApplication = async () => {
    ...
    if (text.value === "") {
      ...
      await axios
        .post("http://localhost:10000/api/application", bodyJson)
        .then((res) => {})
        .catch((err) => {
          alert("error happend");
        });
